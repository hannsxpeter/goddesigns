...
## Head-to-head: the official frontend-design skill (frontend-design-kilnhouse-a/b/c.html/png)

Three isolated same-brief runs using Anthropic's official frontend-design skill (42 lines of guidance, no variance mechanism, no verification; the goddesign standing rule was explicitly waived per run). Findings:

**Convergence, the decisive metric.** Runs B and C independently chose the same direction BY NAME ("porcelain and cobalt", both self-describing it as deliberately-not-terracotta), the same display font (Young Serif), and near-identical hexes (cobalt #2440C0 vs #2340C0; porcelain #F4F4EE vs #F5F3EE). All three share one device recipe under different paint: serif display + grotesque body + mono data readouts (A and B chose the same mono, Spline Sans Mono), a live kiln-status card stack in the hero, a ticker/marquee strip, a giant ghosted footer wordmark, staggered load plus scroll-triggered reveals, and ceramics-pun pricing tiers (A and C identically Cone 04/6/10). The skill's own instruction, "NEVER converge on common choices across generations", is unenforceable: the model has no state across generations and cannot self-randomize. goddesign's benchmark on the same brief: seven runs, seven directions, zero shared ledger axes.

**Audit: 0 of 3 pass.** Every run failed at every viewport with the same signature: 10+ text elements hidden at effective opacity 0 (the scroll-reveal blanking the official skill actively encourages; fd-b's full-page capture renders a hero, then nothing until the footer), real text collisions in each (A: footer watermark over the copyright line; B: schedule board; C: kiln card), sub-24px touch targets on mobile in all three, and horizontal overflow at 375 in B. Nothing catches any of this because the skill has no gate; the run ends when the code is written.

**Fair credit.** Hero-level craft is genuinely good: characterful type pairings, tasteful palettes, domain-aware copy. Against no skill it clearly raises the floor, consistent with the public praise for it. The failures are systemic, not aesthetic: no variance mechanism, encouraged scroll reveals, and no verification.

**The matched 3 vs 3.** A third goddesign run (claude-skill-kilnhouse-c.html/png) was added on the identical brief, host, and prompt shape so both skills have three same-condition runs. goddesign's trio: Terminal Core amber / Letter / dark / mono display / hue 72; Swiss International / Manifesto / light / grotesque / hue 23; Cinematic Dark / Offset Cascade / dark / serif (Cormorant Garamond) / hue 20. Three distinct directions, three distinct macrostructures, three display classes, final audits 3 of 3 green (run C's cycle 1 named real collisions and small targets; the bounded loop fixed exactly those, green by cycle 2). Run C's lock also logged the reroll rule firing: its stated instinct was the cream-terracotta-serif reflex, the same attractor the frontend-design runs orbited. frontend-design's trio: 2 distinct concepts in 3 runs (B and C converged on "porcelain and cobalt" by name, Young Serif both, cobalt hexes one digit apart), all three on one device recipe, final audits 0 of 3. Honest footnotes: goddesign runs B and C landed accents in the same red band (different rows' own palettes; fresh directories carry no cross-run ledger constraint), and both skills' runs independently reached for greenware/bisque/stoneware-style ceramics tier names, a subject-driven copy attractor no variance mechanism governs.
