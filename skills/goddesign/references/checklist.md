...
Blind read, optional (an aid, not a hard gate). The measured audit checks whether the page is broken; it says nothing about whether the page's identity lands. When an image-capable CLI is present (Codex or Claude Code), run the blind post-render critic on the screenshots the audit produced: `sh <skill-root>/scripts/blind-read.sh shot-1280.png shot-375.png`. It shows a fresh process ONLY the pixels, never your code or your DIRECTION LOCK, and returns the page's reconstructed structure, paper band, display class, accent band, signature, subject, and one AI tell as JSON (prompt in `references/blind-read.md`). It runs from an isolated temp directory on anonymized copies of the screenshots, so it sees pixels only: no code, no lock, no repo rules, no method-revealing filenames. This closes the Phase 1 gap where the self-critique is written by the same context that holds the answer key. Compare the reconstruction against the lock: if the blind reader names a different subject than you locked, misses your signature, or returns `identity` "weak", that is a strong signal your Specificity is not landing even though the page renders; revisit Phase 1 axis 4 and decide whether to fix. This is advisory, not an automatic gate failure: calibration (`validation/blind-read-calibration-2026-07.md`) showed the reader recovers subject and signature reliably but does not rank good pages against each other, so a clean recovery is necessary, not sufficient, and a mismatch is a prompt to reconsider, not a verdict. Exit 2 prints `DEGRADED: no blind read`; then rely on your own Phase 3 inspection, with no penalty. Never feed the lock or the code into the blind read; its whole value is that it did not see them.
